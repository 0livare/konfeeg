...
// @ts-expect-error -- This is a js test
function baseEntry(value) {
  return {
    doc: 'test',
    local: value,
    nonprod: value,
...
describe('environment selection', () => {
  it('reads from the local environment', () => {
    const config = createEnvironmentConfig('local', {
      key: { ...baseEntry('shared-value'), local: 'local-value', format: String },
    })
  ...
  it('reads from the nonprod environment', () => {
    const config = createEnvironmentConfig('nonprod', {
      key: { ...baseEntry('nonprod-value'), nonprod: 'nonprod-value', format: String },
    })
    expect(config.key).toBe('nonprod-value')
  })

  it('reads from the prod environment', () => {
    const config = createEnvironmentConfig('prod', {
      key: { ...baseEntry('nonprod-value'), prod: 'prod-value', format: String },
    })
  ...
  it('throws when running in sandbox with no sandbox value and no fallback', () => {
    expect(() =>
      createEnvironmentConfig('sandbox', {
        key: { doc: 'test', format: String, nonprod: 'nonprod-value', prod: 'prod-value' },
      })
    ).toThrow()
  })
})
...
describe('value source precedence', () => {
  ...
  it('processEnv overrides a per-environment value', () => {
    ...
    const config = createEnvironmentConfig('nonprod', {
      key: { doc: 'test', format: String, processEnv: 'TEST_EC_PREC', nonprod: 'env-value', prod: 'env-value' },
    })
  ...
  it('processEnv overrides a static value', () => {
    process.env.TEST_EC_PREC = 'runtime-value'
    const config = createEnvironmentConfig('nonprod', {
      key: { doc: 'test', format: String, processEnv: 'TEST_EC_PREC', value: 'static-value' },
    })
    expect(config.key).toBe('runtime-value')
    delete process.env.TEST_EC_PREC
  })
  ...
  it('per-environment value overrides a static value', () => {
    const config = createEnvironmentConfig('nonprod', {
      key: { doc: 'test', format: String, value: 'static-value', nonprod: 'env-specific', prod: 'env-specific' },
    })
  ...
  it('sandbox-specific value overrides a static value', () => {
    const config = createEnvironmentConfig('sandbox', {
      key: { doc: 'test', format: String, value: 'static-value', sandbox: 'sandbox-specific' },
    })
  ...
  it('uses the per-environment value when processEnv is declared but the runtime var is not set', () => {
    ...
    const config = createEnvironmentConfig('nonprod', {
      key: { doc: 'test', format: String, processEnv: 'TEST_EC_UNSET', nonprod: 'env-value', prod: 'env-value' },
    })
    expect(config.key).toBe('env-value')
  })
  ...
  it('uses the static value when processEnv is declared, the runtime var is not set, and no per-environment value exists', () => {
    delete process.env.TEST_EC_UNSET
    const config = createEnvironmentConfig('nonprod', {
      key: { doc: 'test', format: String, processEnv: 'TEST_EC_UNSET', value: 'static-value' },
    })
    expect(config.key).toBe('static-value')
  })
})
...
describe('format validation', () => {
  ...
  it('throws when a string format receives a non-string value', () => {
    expect(() =>
      createEnvironmentConfig('nonprod', {
